...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            images, orig_caps, orig_lengths, _, _, ids = data_i
        else: 
            images, orig_caps, orig_lengths, gen_caps, gen_lengths, ids = data_i
            image_lengths = None
            
        if torch.cuda.is_available():
            images = images.cuda()
            orig_caps = orig_caps.cuda()
            if image_lengths is not None:
                image_lengths = image_lengths.cuda()
        
        with torch.no_grad():
            if not backbone:
                img_emb, cap_emb = model.forward_emb(images, orig_caps, orig_lengths) 
            else:
                img_emb, cap_emb = model.forward_emb(images, orig_caps, orig_lengths,
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path='/home/ubuntu/paper6/runs', cxc=False):
    """
    ...
    opt.backbone_path = '/root/cvpr26/data/vit_base_patch16_224.augreg_in21k_timm/pytorch_model.bin'
    ...
    data_loader = image_caption_v2t_clip.get_test_loader(split, opt.data_name, tokenizer,
    ...
    end_time = time.time()
    print("calculate encoding time: {}".format(end_time - start_time))
    # img_embs_tensor = torch.tensor(img)
    if cxc:
        ...
        if not fold5:
            ...
            logger.info("Average t2i Recall: %.1f" % ari)
            logger.info("Text to image: %.1f %.1f %.1f %.1f %.1f" % ri)
            message = "split: %s, Image to text: (%.1f, %.1f, %.1f) " % ('test', r[0], r[1], r[2])
            message += "Text to image: (%.1f, %.1f, %.1f) " % (ri[0], ri[1], ri[2])
            message += "rsum: %.1f\n" % rsum
            end = time.time()
            logger.info("calculate similarity time: {}".format(end - start))
            # 这里需要修改路径————————————————————————————————————————————————
            log_file = os.path.join("/root/autodl-tmp/runs/runX/checkpoint", "performance.log")
