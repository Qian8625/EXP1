...
class OSIT(nn.Module):
    def __init__(self, embed_size):
        ...
        # self.aggr_net = TextGuidedTokenAggregator(embed_dim=self.embed_size, num_tokens=self.keeped_patches, num_heads=8, mlp_ratio=1, dropout=0.1)
        
        # 图像分支（有 Saa）
        self.de_decoder_vis = Denoise_Decoder(seq_len_q = self.keeped_patches + 1, seq_len_k=900, hidden_dim=900)

        # 文本分支（无 Saa）
        self.de_decoder_txt = Denoise_Decoder2(seq_len_k=900, hidden_dim=900)
    ...
    def forward(self, base_features, t_features, g_t_features, g_cap_emb, global_feature=None):
        ...
        img_spatial_cro_attention = (g_t_features * img_spatial_embs_norm).sum(dim=-1)
        ...
        emb_v = torch.sqrt(torch.tensor(self.cat_weight))*img_patch + torch.sqrt(torch.tensor(1-self.cat_weight))*concept_v
        ...
        emb_t = torch.sqrt(torch.tensor(self.cat_weight))*t_features + torch.sqrt(torch.tensor(1-self.cat_weight))*concept_t
