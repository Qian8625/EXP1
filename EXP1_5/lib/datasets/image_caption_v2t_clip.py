...
class RawImageDataset(data.Dataset):
    _clip_bundle = None  # legacy, kept for backward compatibility
    """
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train):
        ...
        long_feat_default = os.path.join(opt.data_path, f"LLava_{self.split}_clip_text.npy")
        self.long_caption_feat_path = getattr(opt, "long_caption_feat_path", long_feat_default)
        self.long_caption_features = np.load(self.long_caption_feat_path)
        ...
        # Read Captions (Original captions) - need this before validation
        self.captions = []
        with open(osp.join(loc_cap, '%s_caps.txt' % data_split), 'r', encoding='utf-8') as f:
            for line in f:
                self.captions.append(line.strip())
        
        # Validate that long caption features length matches captions length
        num_long_features = len(self.long_caption_features) 
        num_captions = len(self.captions) 
        num_images = len(self.images)
        print('num_long_features',num_long_features, "num_captions",num_captions, 'num_images',num_images)
        if num_long_features != (num_captions) and num_long_features *5 != num_captions :
            raise ValueError(
                f"Length mismatch: long_caption_features has {num_long_features} entries, "
                f"but captions has {num_captions} entries. "
                f"Images has {num_images} entries. "
                f"Please regenerate the feature file: {self.long_caption_feat_path}"
            )
        ...
        # num_images was already computed during validation above
    ...
    def __getitem__(self, index):
        ...
        original_caption = self.captions[index]
        ...
        if self.split == 'train':
            # Note: train.npy 维度(29000, 512)，所以这里索引img_index
            long_target_ft = torch.tensor(self.long_caption_features[img_index], dtype=torch.float32)
        else:
            # Note: dev和test维度分别为：(5070, 512)，(5000, 512)，都进行了5倍重复处理，所以这里索引index
            long_target_ft = torch.tensor(self.long_caption_features[index], dtype=torch.float32)
        ...
        # print('image_id',image_id,'index',index,'original_caption',original_caption)
        processed_image = self._process_image(im_in)
        image = torch.Tensor(processed_image)
        image = image.permute(2, 0, 1)

        return image, original_target, long_target_ft, index, img_index
...
def process_caption_clip(tokens, train=True):
    output_tokens = []
    deleted_idx = []
...
def collate_fn(data):
    ...
    # 这里直接根据batch 将特征进行堆叠
    generated_targets = torch.stack(generated_captions_list, 0)
    generated_lengths = [feat.shape[0] for feat in generated_captions_list]
