...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train):
        ...
        if hasattr(opt, 'generated_caps_path') and opt.generated_caps_path:
            base_path = os.path.splitext(opt.generated_caps_path)[0]
            # clip_feature_path = f"{base_path}_clip.npy"
            clip_feature_path = "../cvpr26/data/v2t_llava_generate_clip.npy"
            print(clip_feature_path)

            if not os.path.exists(clip_feature_path):
                raise FileNotFoundError(
                    f"Pre-computed CLIP features not found at {clip_feature_path}. "
                    "Please run the `preprocess_clip.py` script first."
                )
            
            self.generated_caps_features = np.load(clip_feature_path)
            logger.info(f"Loaded {len(self.generated_caps_features)} pre-computed CLIP features.")
    ...
    def __getitem__(self, index):
        ...
        # 直接输入对应id 的clip生成特征
        generated_caption_ft = torch.from_numpy(self.generated_caps_features[image_id])
        ...
        return image, original_target, generated_caption_ft, index, img_index
